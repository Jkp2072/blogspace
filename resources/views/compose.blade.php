...
.ho:hover{
   ...
   <!-- https://cdn2.iconfinder.com/data/icons/android-12/512/logout_signout-512.png
    ...
    <form action="{{route('a.composecreate')}}" method="post">
    {{ csrf_field() }}
    ...
    <div class="error">@error('passage'){{ $message }}@enderror</div> 
    <input  type="hidden" name="name" placeholder="{{$info->name}}" value="{{$info->name}}">
